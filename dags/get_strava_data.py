...
dag = DAG(
    ...
    description='A DAG to retrieve data from my strava API daily',
    schedule='@daily')
...
def get_new_strava_data(ti):
    ...
    if res.status_code == 200:
        ...
        request_page_num = 1
        startdate = (datetime.now() - timedelta(days=1)).timestamp()
        #end_date= datetime(2024,2,12).timestamp()
    ...
    #Columns of interest
    selected_fields = ['name', 'distance','moving_time', 'elapsed_time','gear_id', 'total_elevation_gain', 'type', 'sport_type', 'average_speed','max_speed', 'summary_polyline','start_date','timezone','elev_high','elev_low','average_heartrate', 'max_heartrate','average_cadence']
